...
def count_parameters_in_MB(model):
    return sum(np.prod(v.size()) for name, v in model.named_parameters() if "aux" not in name)/1e6


if __name__ == '__main__':
    from mmcls.models import build_backbone
    network_cfg = dict(type='MobilenetV1', implement='local')
    network_cfg = dict(type='tf_mobilenetv3_small_100', implement='timm')
    network_cfg = dict(type='resnet34', implement='torchvision')
    # network_cfg = dict(type='gluon_resnet152_v1s', implement='timm')
    network_cfg = dict(type='resnet34d', implement='timm')
    network = build_backbone(network_cfg)
    network.eval()
    print(f'Param, FLOPs')
    param = count_parameters_in_MB(network)
    flops = comp_multadds(network)
    print(f'{param:.2f} & {flops:.1f}')
